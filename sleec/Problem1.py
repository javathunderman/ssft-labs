...
#     target_property = exists(NUMBER, lambda num)
'''
Solution 3:
'''
...
def check_target(target_value:int):
    target_property = exists(NUMBER, lambda num, target_value=target_value: EQ(num.val, Int(target_value)))
...
odd_rules = forall(ODD, lambda odd: OR(
     OR([TRUE() for i in Basics if not (i % 2)]),
     exists(ODD, lambda odd_prev: odd > odd_prev)
))
...
def target_transformation(target:int):
    ...
    if (target % 2):
        ...
        abstracted_goal = exists(ODD, lambda odd: forall (ODD, lambda odd_prime: odd <= odd_prime))
...
# check_target(14)
check_target_with_abstraction(14)
